...
import com.yoon.rest.webservices.restful_web_services.jpa.PostRepository;
import com.yoon.rest.webservices.restful_web_services.jpa.UserRepository;
...
public class UserJpaResource {
	...
	public EntityModel<User> retriveOneUsers(@PathVariable int id) {
		...
		//Jpa를 사용했기 때문에 Optional객체 반환
		Optional<User> user = userRepository.findById(id);
		//userRepository에서 존재하지않는 id의 Optional<User>를 조회할경우
		//Optional객체의 isEmpty활용
		if (user.isEmpty()) {
	...
	// @PathVariable로 찾을 path 변수이름은 id
	// 특정 id의 유저가 등록한 게시물(Post)을 모두 조회하는 메서드
	@GetMapping("/jpa/users/{id}/posts")
	public List<Post> retrivePostForOneUser(@PathVariable int id) {
		...
		//존재할경우 Optional<User>에서 User를 가져오고 User에서 List<Post>를 가져온다
	...
	public ResponseEntity<User> createUser(@Valid @RequestBody User user) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest()
				...
				.path("/{id}")
				// 3. 쿼리스트링에 넣을 id는 User객체 생성시 자동 생성되는 id 값(seq)을 가져와서 넣는다 ex) 1, 2, 3......
	...
	// @PathVariable로 찾을 path 변수이름은 id
	// 유효성검사후 생성할 객체는 Post
	// 특정 id의 유저가 게시물(Post)을 등록하는 메서드
	@PostMapping("/jpa/users/{id}/posts")
	public ResponseEntity<Object> createPostForOneUser(@PathVariable int id, @Valid @RequestBody Post post) {
		//select u1_0.id,u1_0.birth_date,u1_0.name from user_details u1_0 where u1_0.id=?
		Optional<User> user = userRepository.findById(id);
		//userRepository에서 존재하지않는 id의 Optional<User>를 조회할경우
		//Optional객체의 isEmpty활용
		...
		//Optional<User>가 존재할경우 Post에  User를 넣는다
		post.setUser(user.get());
		
		//select next value for post_seq
		//insert into post (description,user_id,id) values (?,?,?)
		//insert된 Post변수 - Post 엔티티에 특정 User의 id를(user_id컬럼)을 가진 Post를 insert
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest()
				// 2. 현재요청 Url에 추가하고싶은 쿼리스트링지정
				.path("/{id}")
				// 3. 쿼리스트링에 넣을 id는 Post객체 생성시 자동 생성되는 id 값(seq)을 가져와서 넣는다 ex) 1, 2, 3.......
